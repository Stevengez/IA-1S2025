async function fit_predict() {
    ...
    // const features = encAlt.map((_, i) => [encPat[i], encRes[i], encPrice[i]]);
    // const features = encAlt.map((_, i) => [encPrice[i], encRain[i], encEst[i]]);
    // const features = encAlt.map((_, i) => [encAlt[i], encBar[i], encFri[i]]);
    const features = encAlt.map((_, i) => [encPat[i], encEst[i], encType[i]]);
